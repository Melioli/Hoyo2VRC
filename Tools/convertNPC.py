...
class ConvertNonePlayerCharacter(Operator):
    ...
    def execute(self, context):
        ...
        def FixVRCLite():
            ...
            def BoneExists(bone_name):
                return bone_name in bpy.context.object.data.edit_bones

            
            if BoneExists("+PelvisTwistCFA01") and BoneExists("Hips"):
                attachfeets("+PelvisTwistCFA01", "Hips")

            if bpy.context.scene.connect_chest_to_neck:
                if BoneExists("Chest") and BoneExists("Neck"):
                    attachfeets("Chest", "Neck")
            else:
                if BoneExists("Upper Chest") and BoneExists("Neck"):
                    attachfeets("Upper Chest", "Neck")

            if BoneExists("Right arm") and BoneExists("Right elbow"):
            ...
            if BoneExists("+UpperArmTwistRA01") and BoneExists("+UpperArmTwistRA02"):
                attachfeets("+UpperArmTwistRA01", "+UpperArmTwistRA02")

            if BoneExists("+UpperArmTwistLA01") and BoneExists("+UpperArmTwistLA02"):
