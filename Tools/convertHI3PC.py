...
class ConvertHonkaiImpactPlayerCharacter(Operator):
    ...
    def execute(self, context):
                        ...
                        if 'Left leg' in armature.data.edit_bones:
                            if 'Right leg' in armature.data.edit_bones:
                                ...
                                if hips.tail[z_cord] < hips.head[z_cord]:
                                    hips.tail[z_cord] = hips.tail[z_cord] + -2
        ...
        def FixEyes():
            ...
            if "Eye_R" in armature.edit_bones:
                ...
                print("Eye_R not found. Probably using an OldFace Model")

            if "Eye_L" in armature.edit_bones:
                ...
                print("Eye_L not found. Probably using an OldFace Model")
        ...
        def ReparentBones():
            ...
            left_arm = armature.data.edit_bones.get('Left arm')  # Replace 'LeftArm' with the name of your left arm bone
            right_arm = armature.data.edit_bones.get('Right arm')  # Replace 'RightArm' with the name of your right arm bone

            # Iterate over all the bones in the armature
            for bone in armature.data.edit_bones:
                # If the bone name contains 'Bone_Twist_L', reparent it to the left elbow
                if bone.name == 'Twist_L' and left_elbow is not None:
                    bone.parent = left_elbow

                # If the bone name contains 'Bone_Twist_R', reparent it to the right elbow
                if bone.name == 'Twist_R' and right_elbow is not None:
                    bone.parent = right_elbow
                    
                # If the bone name contains 'Bone_Twist_R', reparent it to the right elbow
                elif 'UpperArmTwist_L' in bone.name and left_arm is not None:
